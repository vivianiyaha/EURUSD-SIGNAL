...
SCAN_INTERVAL_SECONDS = 60  # auto-scan every 1 minute while the bot is running
...
# Auto-refresh every 5 seconds while the bot is running so we can check whether
# a full minute has elapsed and a new scan cycle is due (keeps UI responsive
# without re-scanning on every single rerun).
if st.session_state["bot_running"]:
    st_autorefresh(interval=5000, key="auto_refresh_tick")

    now = datetime.now()
    last_scan = st.session_state["last_scan_time"]
    due = last_scan is None or (now - last_scan).total_seconds() >= SCAN_INTERVAL_SECONDS

    if due:
        with st.spinner("Scanning markets..."):
            run_scan_cycle()
    else:
        remaining = SCAN_INTERVAL_SECONDS - (now - last_scan).total_seconds()
        st.caption(f"⏱ Next auto-scan in ~{int(remaining)}s (scanning every {SCAN_INTERVAL_SECONDS}s).")
